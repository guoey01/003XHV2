...
# 输入所有建模参数
vars =[ "gender" ,"address" ,"grip max" ,"IADL" ,"self health status" ,"mental problems","Sleep time","Pain","Hope","MCV","UA","TyG Index"]

# 初始化 session_state 中的 data
# 创建一个空的DataFrame来存储预测数据
if 'data' not in st.session_state:
    st.session_state['data'] = pd.DataFrame(
        columns=[vars[0], vars[1], vars[2], vars[3] ,vars[4] ,vars[5] ,vars[6] ,vars[7] ,vars[8] ,vars[9] ,vars[10] ,vars[11]  ,'Prediction Label' ,'Label'])

# 在主页面上显示数据
st.header('Depression risk of adult patients with digestive_diseases based on SF')
...
#vars =[ "gender" ,"address" ,"grip max" ,"IADL" ,"self health status" ,"mental problems","Sleep time","Pain","Hope","MCV","UA","TyG Index"]
# Input bar 1
a = st.sidebar.number_input(vars[0]+"(1-Male,2-Female)" ,min_value=1 ,max_value=2,value=1)
b = st.sidebar.number_input(vars[1]+"(1-rural areas,2-urban)",min_value=1 ,max_value=2,value=1)
c = st.sidebar.number_input(vars[2]+"(kg)" ,min_value=0.0 ,value=0.0)
d = st.sidebar.number_input(vars[3]+"(0-No,1-Yes)" ,min_value=0 ,max_value=1,value=0)
e = st.sidebar.number_input(vars[4]+"(1-fair,2-good)" ,min_value=1 ,max_value=2,value=1)
f = st.sidebar.number_input(vars[5]+"(0-No,1-Yes)" ,min_value=0 ,max_value=1,value=0)
g = st.sidebar.number_input(vars[6]+"(h)" ,min_value=0.0 ,value=0.0)
h = st.sidebar.number_input(vars[7]+"(0-No,1-Yes)" ,min_value=0 ,max_value=1,value=0)
i = st.sidebar.number_input(vars[8]+"(0-No,1-Yes)" ,min_value=0 ,max_value=1,value=0)
j = st.sidebar.number_input(vars[9]+"(fl)" ,min_value=0.0 ,value=0.0)
k = st.sidebar.number_input(vars[10]+"(mg/dl)" ,min_value=0.0 ,value=0.0)
l = st.sidebar.number_input(vars[11] ,min_value=0.0 ,value=0.0)
...
if st.sidebar.button("Submit"):
    ...
    X = pd.DataFrame([[a, b, c ,d ,e ,f,g,h,i,j,k,l]],
                     columns=[vars[0], vars[1], vars[2], vars[3] ,vars[4] ,vars[5],vars[6],vars[7],vars[8],vars[9],vars[10],vars[11]])
    ...
    new_data = pd.DataFrame([[a, b, c ,d ,e ,f, g,h,i,j,k,l,result_prob_pos, None]],
...
if uploaded_file is not None:
    ...
    # 检查是否所有必需的列都存在
    missing_cols = [col for col in [vars[0], vars[1], vars[2], vars[3] ,vars[4] ,vars[5],vars[6],vars[7],vars[8],vars[9],vars[10],vars[11]] if
    ...
    if missing_cols:
        ...
        for _, row in df.iterrows():
            ...
            new_data = pd.DataFrame([[row[vars[0]], row[vars[1]], row[vars[2]] ,row[vars[3]] ,row[vars[4]]
                                      ,row[vars[5]], row[vars[6]],row[vars[7]],row[vars[8]],row[vars[9]],row[vars[10]],row[vars[11]],result_prob, label]],
                                    columns=st.session_state['data'].columns)
            st.session_state['data'] = pd.concat([st.session_state['data'], new_data], ignore_index=True)
